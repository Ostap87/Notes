...
include --> runs only the methods that are passed, everything else in that class will be ignored

exclude --> runs everything except for the exluced methods in that class
...
// verbose="2" - means how many lines of details will print out after executing
'
...
// must be included @BeforeMethod and @AfterMethod (groups={"Smoke"}) or
// (alwaysRun = true) with driver inside
...
sequence of calls when we run testng test using command line:
    ...
    which is pointing to testng_runner.xml. 
    That means pom xml will trigger testng_runner.xml 
...
1. terminal -> go to the current project
2. mvn test -Drunner = nameOfFile.xml
